...
Plot_2224: This script compares the 2022–2024 CTD density profiles by computing σ₀, aligning the deep parts of the profiles using bottom values, then saving and plotting the aligned density differences between years.

3. mixing_scan_u_1314,1516: This script estimates vertical diffusivity k(u) from two aligned density profiles and basin hypsometry by scanning different depths, then saves and plots the resulting mixing profiles.

mixing_scan_u_2224: This script estimates vertical diffusivity for 2022–2024 by combining basin hypsometry with both aligned and raw density-profile differences, then compares the resulting k(u) profiles across depth and time.

4. Plot_mixing_u: This script reads the diffusivity profiles from different stagnation periods, removes selected unreliable depths, and plots the vertical diffusivity k(u) curves together for comparison.

Compare CTD: 

1. ompare_2017_2019: This script compares selected CTD profiles from 2017–2019 by calculating vertical profiles and layer-mean values of σ₀, CT, SA, and dissolved oxygen, then quantifies year-to-year changes and plots them in a four-panel figure.
